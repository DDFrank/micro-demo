...
func (s *service) saveTokenToCache(subject *Subject, val string) (err error) {
	// 保存
	if err = redisClient.Set(tokenIDKeyPrefix+subject.ID, val, tokenExpiredDate).Err(); err != nil {
...
func (s *service) delTokenFromCache(subject *Subject) (err error) {
	// 保存
	if err = redisClient.Del(tokenIDKeyPrefix + subject.ID).Err(); err != nil {
...
func (s *service) getTokenFromCache(subject *Subject) (token string, err error) {
	// 获取
	tokenCached, err := redisClient.Get(tokenIDKeyPrefix + subject.ID).Result()
	if err !=nil {
...
//parseToken 解析token
func (s *service) parseToken(tokenString string) (c *jwt.StandardClaims, err error) {
